  ...
  if (!isCompleted) {
    ...
    if (currentUserProgress.wallet_address) {
      try {
        ...
        type MintTx = { hash?: string } | string;
        const tx: MintTx = await mintByte(currentUserProgress.wallet_address, amount);
        // writeContract often returns an object with a `hash` property; in some cases a plain hash string
